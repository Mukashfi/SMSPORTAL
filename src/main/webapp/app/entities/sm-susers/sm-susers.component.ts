...
export class SMSusersComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.sMSusersService
      ...
      .subscribe(
        (res: HttpResponse<ISMSusers[]>) => this.paginateSMSusers(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/sm-susers'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateSMSusers(data: ISMSusers[], headers: HttpHeaders) {
